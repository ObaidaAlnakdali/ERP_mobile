...
const Item = ({ name, rate, id, kpiList, kpi_id }) => {
  ...
  const finalFunction = async () => {
    await axios.get(`http://192.168.0.109:8000/api/employees/${id}`).then(response => {
      setEmployee({
  ...
  const fifi = async () => {
    await axios.post(`http://192.168.0.109:8000/api/employees/${id}`, employee).then(response => {
...
function KPI({ route }) {
  ...
  const getKpiEmployee = async () => {
    await axios.get(`http://192.168.0.109:8000/api/employees/validKPIS/${id}`).then(response => {
